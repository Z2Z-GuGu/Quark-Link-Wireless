        ...
        case BLE_SETTING_CHANNEL:
            switch(p_evt->type)
            {
                case BLE_UART_EVT_TX_NOTI_DISABLED:
                    ...
                    SYS_STATE_BITMAP &= ~SYS_BLE_CONNECT_STATE_Bit;
                    break;
        ...
        case BLE_UART_CHANNEL:
            switch(p_evt->type)
            {
                ...
                case BLE_UART_EVT_BLE_DATA_RECIEVED:
                    ...
                    // app_drv_fifo_write(&UART_to_BLE_fifo, (uint8_t *)p_evt->data.p_data, &to_write_length);
                    if(SYS_STATE_BITMAP & SYS_BLE_to_UART_EN_Bit)
                    {
                        app_drv_fifo_write(&BLE_to_UART_fifo, (uint8_t *)p_evt->data.p_data, &to_write_length);
                    }
                    break;
                default:
                    break;
                    ...
                    case BLE_UART_CHANNEL:
                    {
                        ...
                        if(!ble_uart_notify_is_ready(BLE_ConnList.connHandle))
                        {
                            if(BLE_ConnList.connHandle == GAP_CONNHANDLE_INIT)
                            {
                                ...
                                app_drv_fifo_flush(&UART_to_BLE_fifo);
                            }
                            break;
                        }
                        read_length = ATT_GetMTU(BLE_ConnList.connHandle) - 3;

                        if(app_drv_fifo_length(&UART_to_BLE_fifo) >= read_length)
                        {
                            PRINT("FIFO_LEN:%d\r\n", app_drv_fifo_length(&UART_to_BLE_fifo));
                            result = app_drv_fifo_read(&UART_to_BLE_fifo, to_be_send_buf, &read_length);
...
void ble_fifo_init()
{
    ...
    // app_drv_fifo_init(&UART_to_BLE_fifo, ble_tx_buffer, BLE_TX_BUFFER_LENGTH);
